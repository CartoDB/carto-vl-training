...
[Ramp can find them for you](https://carto.com/developers/carto-vl/guides/data-driven-visualizations-part-1/#what-is-a-ramp). 

To demonstrate this we'll make a new map. The new dataset we're using has numeric data: a column named `dn` that contains a population density value for each of it's point locations. 
...
Our points with low population density are hard to see, since they are close to the same color as the basemap. We can make them more legible by changing the first value in our color `ramp`. 
...
What if we do want to put our attribute values into buckets, but we also want to make sure they're bucketed by a statistical classification method? The `ramp` function can do that automatically for us too. 
...
11. Change your `color` property to this:
    ...
    * The number of categories is taken from the number of colors we define in the `ramp` function's second parameter.
...
16. Now add this width property to the `viz`, to change the default point size:
    ...
    * There's a number-type column named `total_damage` in the rail safety dataset.
      * It contains the amount of damage the accident caused in dollars.
